    ...
    <div className="fixed bottom-16 z-[9998] -translate-x-1/2" style={{ left }}>
      {isCollapsed ? (
        <button
          ...
          className="hover:bg-gray-5 active:bg-gray-10 flex h-[74px] w-[74px] items-center justify-center rounded-[20px] bg-white shadow-lg transition-colors"
        >
          <PlusIcon className="text-gray-90 h-[18px] w-[18px]" />
        </button>
      ) : (
        <div className="relative flex items-center justify-center gap-[50px] rounded-[26px] bg-white px-[50px] py-[31px] shadow-lg">
          {/* 접기 버튼 (우측 상단) */}
          <button
            type="button"
          ...
          <ActionButton
            icon={
              <FloatingLinkIcon className="text-gray-90 h-[31px] w-[37px]" />
            }
            label="링크 추가"
          ...
          <ActionButton
            icon={
              <FloatingFileIcon className="text-gray-90 h-[31px] w-[37px]" />
            }
            label="파일 추가"
